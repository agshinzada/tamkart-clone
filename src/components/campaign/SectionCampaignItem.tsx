...
const SectionCampaignItem = ({ data }: { data: CampaignItemType }) => {
  ...
  return (
    <div className="laptop:pb-12 pb-8 tablet:border-b tablet:border-b-csecondary laptop:border-none">
      <Link to={`/campaigns/${data.id}`} className="group">
        <div className="flex flex-col laptop:flex-col tablet:flex-row gap-10 laptop:gap-0">
          ...
          <div className="text-white flex flex-col justify-between laptop:mt-8 mt-0">
            ...
            <button
              type="button"
              className="hidden tablet:block laptop:hidden px-6 py-3 text-sm h-fit w-fit bg-csecondary text-white rounded-xl font-semibold group-hover:text-black group-hover:bg-cprimary transition-all duration-300"
